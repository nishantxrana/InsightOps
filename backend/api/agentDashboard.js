...
router.get("/agentic-score", async (req, res) => {
  ...
  try {
    ...
    const score = Math.round(
      caching * 0.25 + ruleUsage * 0.2 + learning * 0.2 + workflows * 0.15 + autonomy * 0.2
    );
    ...
    res.json({
      ...
      components: {
        ...
        ruleUsage: { score: Math.round(ruleUsage * 0.2), weight: "20%" },
        learning: { score: Math.round(learning * 0.2), weight: "20%" },
        workflows: { score: Math.round(workflows * 0.15), weight: "15%" },
        autonomy: { score: Math.round(autonomy * 0.2), weight: "20%" },
      },
    });
  } catch (error) {
