...
export default function FilterDropdown({
  ...
  const themeClasses = {
    blue: {
      button:
        "border-border dark:border-[#1a1a1a] focus:ring-muted focus:border-border hover:border-muted-foreground",
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className={`absolute top-full left-0 right-0 mt-1 bg-card dark:bg-[#111111] border rounded-lg shadow-lg z-50 py-1 min-w-[140px] ${currentTheme.dropdown}`}
        >
          {options.map((option) => (
            <button
              key={option.value}
              ...
              className={`w-full text-left px-3 py-2 text-xs transition-colors ${
                value === option.value
                  ...
                  : `text-foreground ${currentTheme.option}`
              }`}
            >
              {option.label}
            </button>
