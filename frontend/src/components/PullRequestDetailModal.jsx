...
const PullRequestDetailModal = ({ pullRequest, isOpen, onClose }) => {
  ...
  const isReadyToMerge =
    isActive && approvedReviewers.length > 0 && rejectedReviewers.length === 0 && !hasConflicts;
      ...
      <div className="bg-card dark:bg-[#111111] rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-2xl lg:max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col border border-border dark:border-[#1a1a1a]">
        ...
        {isActive && isReadyToMerge && (
          <div className="px-6 py-2 bg-emerald-100 dark:bg-emerald-950/50 border-b border-emerald-200 dark:border-emerald-800 flex items-center gap-2 text-emerald-800 dark:text-emerald-200">
            ...
            <span className="text-xs text-emerald-600 dark:text-emerald-300">
              — {approvedReviewers.length} approval{approvedReviewers.length > 1 ? "s" : ""}, no
              blockers
            </span>
          </div>
        )}
        ...
        {isActive && rejectedReviewers.length > 0 && !hasConflicts && (
          <div className="px-6 py-3 bg-muted border-b border-red-200 dark:border-red-800 flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">
              — {rejectedReviewers.length} reviewer{rejectedReviewers.length > 1 ? "s" : ""}{" "}
              requested changes
            </span>
          </div>
        )}
          ...
          !isBlocked && (
            <div className="px-6 py-2 bg-muted border-b border-border flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                — {waitingReviewers.length} reviewer{waitingReviewers.length > 1 ? "s" : ""} pending
              </span>
            </div>
          )}
        ...
        <div className="overflow-y-auto flex-1">
          <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              ...
              {/* Reviewers */}
              <div
                className={`p-3 rounded-lg bg-muted ${hasNoReviewers ? "border border-orange-400 dark:border-orange-700" : ""}`}
              >
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                ...
                <p
                  className={`font-medium text-sm ${hasNoReviewers ? "text-orange-700 dark:text-orange-300" : "text-foreground"}`}
                >
                  {hasNoReviewers ? "None assigned" : `${reviewers.length} assigned`}
                </p>
                {approvedReviewers.length > 0 && (
                  <p className="text-xs text-emerald-600 dark:text-emerald-400">
                    {approvedReviewers.length} approved
                  </p>
                )}
              </div>
              ...
              {/* Merge Status */}
              <div
                className={`p-3 rounded-lg bg-muted ${hasConflicts ? "border border-red-400 dark:border-red-700" : ""}`}
              >
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                ...
                <p
                  className={`font-medium text-sm ${hasConflicts ? "text-red-700 dark:text-red-300" : "text-foreground"}`}
                >
                  {hasConflicts
                    ? "Has conflicts"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {reviewers.map((reviewer, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`flex items-center justify-between p-2 rounded-lg border ${voteInfo.bg}`}
                      >
            ...
            <details className="group">
              <summary className="flex items-center justify-between cursor-pointer bg-muted rounded-lg p-4 border border-border hover:bg-muted/80 transition-colors">
                ...
                <svg
                  className="h-5 w-5 text-blue-600 dark:text-blue-400 transition-transform group-open:rotate-180"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
