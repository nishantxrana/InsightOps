...
export default function TimeRangeSelector({ value, onChange }) {
  ...
  const handleApplyCustomRange = () => {
    if (startDate && endDate) {
      const now = new Date();
      if (endDate > now) {
        toast({ title: "Invalid Date Range", description: "End date cannot be in the future.", variant: "destructive" });
        return;
      }
      const diffDays = Math.ceil(Math.abs(endDate - startDate) / (1000 * 60 * 60 * 24));
      if (diffDays > 90) {
        toast({ title: "Date Range Too Large", description: "Please select 90 days or less.", variant: "destructive" });
        return;
      }
      const from = startDate < endDate ? startDate : endDate;
      const to = startDate < endDate ? endDate : startDate;
      onChange({ from, to, label: `${format(from, 'MMM dd')} - ${format(to, 'MMM dd')}`, value: 'custom' });
      setShowCustom(false);
    }
  };
      ...
      <div className="flex items-center gap-2">
        <Popover open={isOpen} onOpenChange={setIsOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" className="justify-between min-w-[140px] bg-background rounded-full touch-manipulation">
              ...
              {TIME_RANGES.map((range) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-sm rounded-md transition-colors touch-manipulation ${
                    value?.value === range.value ? 'bg-primary text-primary-foreground' : 'hover:bg-muted active:bg-muted'
                  }`}
                >
                  {range.label}
                </button>
        ...
        <Sheet open={showCustom} onOpenChange={setShowCustom}>
          <SheetContent side="bottom" className="h-auto max-h-[85dvh] rounded-t-2xl">
            ...
            <div className="space-y-4 pb-4">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setShowCustom(false)} className="flex-1 touch-manipulation">
                  Cancel
                </Button>
                <Button onClick={handleApplyCustomRange} disabled={!startDate || !endDate} className="flex-1 touch-manipulation">
                  Apply
                </Button>
              </div>
            </div>
  ...
  return (
    <div className="flex items-center gap-2">
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" className="justify-between min-w-[140px] bg-background rounded-full">
            <span className="text-sm">{getCurrentLabel()}</span>
            <ChevronDown className="h-4 w-4 ml-2 opacity-50" />
          </Button>
        </PopoverTrigger>
            ...
            {TIME_RANGES.map((range) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-sm rounded-md transition-colors ${
                  value?.value === range.value ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'
                }`}
              >
                {range.label}
              </button>
      ...
      {showCustom && (
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleApplyCustomRange} disabled={!startDate || !endDate} size="sm">Apply</Button>
