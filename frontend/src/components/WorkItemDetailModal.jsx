  ...
  const modalContent = (
    ...
    >
      <div className="bg-card dark:bg-[#111111] rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-2xl lg:max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden border border-border dark:border-[#1a1a1a] flex flex-col">
        ...
        {needsAttention && (
          <div
            className={`px-6 py-2 flex items-center gap-2 text-sm font-medium border-b ${
              isOverdue
          ...
          >
            <AlertTriangle
              className={`h-4 w-4 ${isOverdue ? "text-red-500" : isCritical ? "text-orange-500" : "text-amber-500"}`}
            />
            {isOverdue && "Overdue"}
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStateColor(state)}`}
                >
                  {state}
                </span>
                {isHighPriority && (
        ...
        <div className="overflow-y-auto flex-1">
          <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              {/* Assignee */}
              <div
                className={`p-3 rounded-lg bg-muted ${assignee === "Unassigned" ? "border border-amber-400 dark:border-amber-700" : ""}`}
              >
                ...
                <p
                  className={`font-medium text-sm truncate ${assignee === "Unassigned" ? "text-amber-700 dark:text-amber-300" : "text-foreground"}`}
                >
                  {assignee}
                </p>
              </div>
              ...
              {/* Priority */}
              <div
                className={`p-3 rounded-lg bg-muted ${isHighPriority ? "border border-red-400 dark:border-red-700" : ""}`}
              >
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                  {getPriorityIcon(priority)}
                  Priority
                ...
                <p
                  className={`font-medium text-sm ${isHighPriority ? "text-red-700 dark:text-red-300" : "text-foreground"}`}
                >
                  {getPriorityText(priority)}
                </p>
              </div>
              ...
              {/* Due Date */}
              <div
                className={`p-3 rounded-lg bg-muted ${isOverdue ? "border border-red-400 dark:border-red-700" : ""}`}
              >
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                ...
                <p
                  className={`font-medium text-sm ${isOverdue ? "text-red-700 dark:text-red-300" : "text-foreground"}`}
                >
            ...
            <div>
              <h4 className="font-medium text-foreground mb-2 flex items-center gap-2">
                Description
                {!description && (
                  <span className="text-xs text-muted-foreground font-normal">(none provided)</span>
                )}
              </h4>
              {description ? (
                <div
