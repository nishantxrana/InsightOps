...
const NotificationHistory = () => {
  ...
  const fetchNotifications = async (loadMore = false) => {
    try {
      ...
      const userMessage =
        error.userMessage || error.message || "Failed to load notifications. Please try again.";
    ...
    <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {!loading &&
              ((counts.overdue || 0) + (counts["idle-pr"] || 0) > 0 ? (
                ...
                notifications.some((n) => n.type === "build" && n.subType === "failed") ? (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full bg-muted">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <Select value={activeTab} onValueChange={setActiveTab}>
            ...
            <SelectContent>
              ...
              <SelectItem value="build">
                Builds {counts.build ? `(${counts.build})` : ""}
              </SelectItem>
              <SelectItem value="release">
                Releases {counts.release ? `(${counts.release})` : ""}
              </SelectItem>
              <SelectItem value="work-item">
                Work Items {counts["work-item"] ? `(${counts["work-item"]})` : ""}
              </SelectItem>
              <SelectItem value="pull-request">
                PRs {counts["pull-request"] ? `(${counts["pull-request"]})` : ""}
              </SelectItem>
              <SelectItem
                value="overdue"
                className={counts.overdue > 0 ? "text-red-600 font-medium" : ""}
              >
                ⚠️ Overdue Work Items {counts.overdue ? `(${counts.overdue})` : ""}
              </SelectItem>
              <SelectItem
                value="idle-pr"
                className={counts["idle-pr"] > 0 ? "text-amber-600 font-medium" : ""}
              >
        ...
        <TabsList className="hidden sm:inline-flex flex-wrap h-auto gap-1 p-1">
          ...
          >
            All{" "}
            {counts.total ? <span className="ml-1 text-xs opacity-70">({counts.total})</span> : ""}
          </TabsTrigger>
          <TabsTrigger
            value="build"
            className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
          >
            Builds{" "}
            {counts.build ? <span className="ml-1 text-xs opacity-70">({counts.build})</span> : ""}
          </TabsTrigger>
          <TabsTrigger
            value="release"
            className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
          >
            Releases{" "}
            {counts.release ? (
              <span className="ml-1 text-xs opacity-70">({counts.release})</span>
            ) : (
              ""
            )}
          </TabsTrigger>
          ...
          >
            Work Items{" "}
            {counts["work-item"] ? (
              <span className="ml-1 text-xs opacity-70">({counts["work-item"]})</span>
            ) : (
              ""
            )}
          </TabsTrigger>
          ...
          >
            PRs{" "}
            {counts["pull-request"] ? (
              <span className="ml-1 text-xs opacity-70">({counts["pull-request"]})</span>
            ) : (
              ""
            )}
          </TabsTrigger>
          ...
          >
            ...
            {counts.overdue ? (
              <span
                className={`ml-1 text-xs ${counts.overdue > 0 ? "text-red-600 dark:text-red-400" : "opacity-70"}`}
              >
                ({counts.overdue})
              </span>
            ) : (
          ...
          >
            ...
            {counts["idle-pr"] ? (
              <span
                className={`ml-1 text-xs ${counts["idle-pr"] > 0 ? "text-amber-600 dark:text-amber-400" : "opacity-70"}`}
              >
                ({counts["idle-pr"]})
              </span>
            ) : (
          ...
          {loading ? (
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {filteredNotifications.map((notification) => {
                ...
                const isWarning =
                  notification.type === "work-item" && notification.subType === "updated";
                const isSuccess =
                    ...
                    <AccordionTrigger className="hover:no-underline px-3 sm:px-6 py-3 sm:py-4 [&[data-state=open]]:border-b grid grid-cols-[1fr_auto] gap-2 items-center">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-left min-w-0">
                          ...
                          {notification.subType && (
                            <Badge
                              ...
                              className={`capitalize text-xs rounded-full ${
                                notification.subType === "succeeded" ||
                                notification.subType === "completed"
                        ...
                        <div className="flex-1 min-w-0">
                          <h3
                            className={`font-semibold text-sm sm:text-base mb-1 truncate ${isUnread ? "text-foreground" : "text-muted-foreground"}`}
                          >
                            {notification.title}
                          </h3>
                    ...
                    <AccordionContent className="px-3 sm:px-6 pb-4 pt-4">
                      <div className="space-y-4 overflow-hidden w-full">
                        ...
                        {notification.metadata &&
                          (notification.type === "build" || notification.type === "release") && (
                                  ...
                                  {notification.metadata.reason && (
                                    <div className="flex items-start gap-2 min-w-0">
                                      ...
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <p className="font-medium text-xs capitalize break-words">
                                          {notification.metadata.reason
                                            .replace(/([A-Z])/g, " $1")
                                            .trim()}
                                        </p>
                        ...
                        {notification.type === "release" &&
                          notification.metadata?.failedTasks &&
                          notification.metadata.failedTasks.length > 0 && (
                            <div className="space-y-3">
                              <div className="flex items-center gap-2 text-sm font-medium text-destructive">
                                <span>
                                  Failed Tasks ({notification.metadata.failedTasks.length})
                                </span>
                              </div>
                              <div className="space-y-2">
                                {notification.metadata.failedTasks.map((task, idx) => (
                                  <div
                                    key={idx}
                                    className="border border-destructive/20 rounded-md p-3 bg-destructive/5"
                                  >
                        ...
                        {notification.type === "work-item" && notification.metadata && (
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-3 text-sm bg-muted/30 p-3 sm:p-4 rounded-md overflow-hidden">
                            ...
                            {notification.metadata.workItemType &&
                              typeof notification.metadata.workItemType !== "object" && (
                                <div className="flex items-start gap-2 min-w-0">
                            ...
                            {notification.metadata.state &&
                              typeof notification.metadata.state !== "object" && (
                                <div className="flex items-start gap-2 min-w-0">
                            ...
                            {notification.metadata.assignedTo &&
                              typeof notification.metadata.assignedTo !== "object" && (
                                <div className="flex items-start gap-2 min-w-0">
                                  <User className="h-4 w-4 text-muted-foreground mt-0.5 shrink-0" />
                                  <div>
                            ...
                            {notification.metadata.priority && (
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <div>
                                  ...
                                  <p className="font-medium break-all">
                                    {notification.metadata.priority === 1
                                      ? "Critical"
                                      : notification.metadata.priority === 2
                                        ? "High"
                                        : notification.metadata.priority === 3
                                          ? "Medium"
                                          : "Low"}
                                  </p>
                                </div>
                              ...
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <div>
                                  ...
                                  <p className="font-medium truncate">
                                    {notification.metadata.areaPath.split("\\").pop()}
                                  </p>
                                </div>
                              </div>
                            )}
                            {notification.metadata.iterationPath && (
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <div>
                                  ...
                                  <p className="font-medium truncate">
                                    {notification.metadata.iterationPath.split("\\").pop()}
                                  </p>
                                </div>
                              </div>
                            )}
                        ...
                        {/* Work Item Changes */}
                        {notification.type === "work-item" &&
                          notification.subType === "updated" &&
                          notification.metadata?.changes &&
                          notification.metadata.changes.length > 0 && (
                            <div className="space-y-2">
                              ...
                              <div className="space-y-1">
                                {notification.metadata.changes.map((change, idx) => (
                                  <div key={idx} className="text-xs bg-muted/50 p-2 rounded">
                                    ...
                                    <p
                                      className="text-primary font-medium break-words"
                                      style={{
                                        wordBreak: "break-word",
                                        overflowWrap: "break-word",
                                      }}
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2 text-sm font-medium">
                                <span className="text-destructive">
                                  {notification.metadata.count} Overdue Items
                                </span>
                              </div>
                              <Button
                                size="sm"
                            ...
                            <div className="space-y-2 max-h-96 overflow-y-auto">
                              {notification.metadata.items.map((item, idx) => (
                                <div
                                  key={idx}
                                  className="border rounded-md p-3 bg-background hover:bg-muted/50 transition-colors"
                                >
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2 text-sm font-medium">
                                <span className="text-orange-600">
                                  {notification.metadata.count} Idle Pull Requests
                                </span>
                              </div>
                              <Button
                                size="sm"
                                ...
                                >
                                  <div className="flex items-start justify-between gap-2 mb-2">
                                    ...
                                    {pr.idleDays > 0 && (
                                      <Badge
                                        variant="outline"
                                        className="text-xs flex-shrink-0 border-orange-600 text-orange-600"
                                      >
                                        {pr.idleDays}d idle
                                      </Badge>
                                    )}
